...
def fit_one_epoch(model_rpn, model_all, loss_history, eval_callback, callback, epoch, epoch_step, epoch_step_val, gen, gen_val, Epoch, anchors, bbox_util, roi_helper, save_period, save_dir):
    ...
    with tqdm(total=epoch_step,desc=f'Epoch {epoch + 1}/{Epoch}',postfix=dict,mininterval=0.3) as pbar:
        for iteration, batch in enumerate(gen):
            ...
            # print(loss_class)
            # print(epoch*epoch_step+iteration)
            write_log(callback, ['train_total_loss_iter','train_rpn_cls_loss_iter', 'train_rpn_reg_loss_iter',
                                 'train_detection_cls_loss_iter', 'train_detection_reg_loss_iter'], loss_class,
                      epoch*epoch_step+iteration)
    ...
    avg_loss = [total_loss / (iteration + 1), rpn_cls_loss / (iteration + 1), rpn_loc_loss / (iteration + 1),
                    roi_cls_loss / (iteration + 1), roi_loc_loss / (iteration + 1)]
    write_log(callback, ['train_total_loss_epoch', 'train_rpn_cls_loss_epoch', 'train_rpn_reg_loss_epoch',
                             'train_detection_cls_loss_epoch', 'train_detection_reg_loss_epoch'], avg_loss, epoch)
    ...
    with tqdm(total=epoch_step_val, desc=f'Epoch {epoch + 1}/{Epoch}',postfix=dict,mininterval=0.3) as pbar:
        for iteration, batch in enumerate(gen_val):
            ...
            write_log(callback, ['test_total_loss_iter', 'test_rpn_cls_loss_iter', 'test_rpn_reg_loss_iter', 'test_detection_cls_loss_iter',
                                 'test_detection_reg_loss_iter'], loss_class, epoch*epoch_step_val+iteration)

            val_loss += loss_class[0]
            val_rpn_cls_loss += loss_class[1]
            val_rpn_loc_loss += loss_class[2]
            val_roi_cls_loss += loss_class[3]
            val_roi_loc_loss += loss_class[4]
            pbar.set_postfix(**{'total' : val_loss / (iteration + 1)})
            pbar.update(1)

    avg_val_loss = [val_loss / (iteration + 1), val_rpn_cls_loss / (iteration + 1), val_rpn_loc_loss / (iteration + 1),
                    val_roi_cls_loss / (iteration + 1), val_roi_loc_loss / (iteration + 1)]
    write_log(callback, ['test_total_loss_epoch', 'test_rpn_cls_loss_epoch', 'test_rpn_reg_loss_epoch',
                             'test_detection_cls_loss_epoch', 'test_detection_reg_loss_epoch'],
                  avg_val_loss, epoch)


    logs = {'loss': total_loss / epoch_step, 'val_loss': val_loss / epoch_step_val}
    loss_history.on_epoch_end([], logs)
    eval_callback.on_epoch_end(epoch, logs, callback)
